...
- **release.yml**（打 `v*` tag）：装 SDK/NDK + gomobile，`gomobile bind` 出 AAR，用 `mvn deploy:deploy-file` 发到 **GitHub Packages（Maven）**，坐标 `com.cca2878:gtlv:<tag 去 v>`——这就是 Android 的「wheel」式分发。发版：`git tag v0.1.0 && git push --tags`。
