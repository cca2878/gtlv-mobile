...
从本仓库的 [Release](../../releases) 下载 `gtlv.aar`（CI 用 gomobile 构建并附在 Release 上），放入 App 模块：

```gradle
// app/build.gradle
dependencies {
    implementation files("libs/gtlv.aar")
}
```

> 消费方**不需要**安装 Go / gomobile / NDK——AAR 已是编好的二进制（含 arm64-v8a 与 x86_64 两个 ABI）。
...
- **ci.yml**（push/PR）：纯 Go 校验（gofmt/vet/build against 上游库），不需 gomobile/NDK，快。
- **release.yml**（打 `v*` tag）：装 SDK/NDK + gomobile，`gomobile bind` 出 AAR 并附到 GitHub Release——这就是 Android 的「wheel」式分发。
