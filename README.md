...
发布在 **GitHub Packages（Maven）**，用干净的坐标 `io.github.cca2878:gtlv-mobile:<version>` 引入，享受 Gradle 的版本/依赖管理。GitHub Packages 的 Maven 仓即便是公开包也要求认证，故消费方需一个具 `read:packages` 权限的 GitHub token。
...
```kotlin
dependencies {
    implementation("io.github.cca2878:gtlv-mobile:0.1.0")
}
```
...
> 消费方**不需要**安装 Go / gomobile / NDK——AAR 已是编好的二进制（含 arm64-v8a 与 x86_64 两个 ABI）。此外每次 CI 也会把 `gtlv-mobile.aar` 作为 **workflow 制品**上传，可直接从任意一次运行下载拿去真机测（见 [CI](#ci)）。
...
make bind-android  # → gtlv-mobile.aar（+ gtlv-mobile-sources.jar）
```
...
- **snapshot.yml**（push 到 dev）：bind 出 AAR，发 `io.github.cca2878:gtlv-mobile:<VERSION>-SNAPSHOT` 到 GitHub Packages（`VERSION` 文件是当前开发版本号）。Gradle 用 `0.1.0-SNAPSHOT` 即可拉最新 dev 构建。
- **release.yml**（在 main 上打 `v*` tag）：bind 出 AAR，发**不可变正式版** `io.github.cca2878:gtlv-mobile:<tag 去 v>`。发版：`git tag v0.1.0 && git push origin v0.1.0`，随后把 `VERSION` bump 到下一个开发版号。
